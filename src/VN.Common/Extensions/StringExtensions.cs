...
namespace System
{
    public static class StringExtensions
    {
        ...
        private static readonly char[] WordBreakChars = new char[] { ' ', '.', '!', ',', ';', ':', '-', '"', '\'', ']', '}' };
        ...
        private static readonly char[] WrapChars = new char[] { ' ', '\t', '\r', '\n', '-' };
        ...
        public static IEnumerable<string> WordWrap(this string text, int columns)
        {
            ...
            while (remainingLength > columns)
            {
                ...
                var lastSplitCharIdx = text.LastIndexOfAny(WrapChars, i + columns, columns);
                ...
                var lineLength = lastSplitCharIdx - i;
        ...
        public static bool CaseInsensitiveEquals(this string self, params string[] compare)
        {
            foreach (var t in compare)
            {
                ...
                if (string.Equals(self, t, StringComparison.OrdinalIgnoreCase)) return true;
            }
            return false;
        }
        ...
        public static string NullOrEmptyCoalesce(this string self, string coalesce)
        {
            ...
            // NOTE: This is the kicker over String.IsNullOrEmpty()
            return self.Trim().Length == 0 ? coalesce : self;
        }
        ...
        public static string TrimOrCoalesce(this string self, string coalesce)
        {
            ...
            return trimmed.Length == 0 ? coalesce : trimmed;
        }
        ...
        public static string RemoveIfStartsWith(this string value, params string[] startsWith)
        {
            ...
            foreach (var t in startsWith.Where(t => !string.IsNullOrEmpty(t)).Where(value.StartsWith))
            {
                return value.Length == t.Length ? string.Empty : value.Substring(t.Length);
            }
        ...
        public static string RemoveIfStartsWith(this string value, string startsWith, StringComparison comparisonType)
        {
            ...
            if (!value.StartsWith(startsWith, comparisonType)) return value;
            return value.Length == startsWith.Length ? string.Empty : value.Substring(startsWith.Length);
        }
        ...
        public static string RemoveIfEndsWith(this string value, params string[] endsWith)
        {
            ...
            foreach (var t in endsWith.Where(t => !string.IsNullOrEmpty(t)).Where(value.EndsWith))
            {
                return value.Substring(0, value.Length - t.Length);
            }
        ...
        public static string RemoveIfEndsWith(this string value, string endsWith, StringComparison comparisonType)
        {
            ...
            return value.EndsWith(endsWith, comparisonType) ? value.Substring(0, value.Length - endsWith.Length) : value;
        }
        ...
        /// <summary>
        /// Parses a list of name=value pairs, delimited by <paramref name="delimiter"/>.  Values may be quoted.  
        /// Names must follow C# identifier
        /// rules.
        /// </summary>
        ...
        public static Dictionary<string, string> ParseNameValues(this string line, char delimiter)
        {
            ...
            while (i < s.Length)
            {
                ...
                while (i < s.Length)
                {
                    ...
                    if (s[i] != '_') return props;
                    ++i;

                }
                if (i >= s.Length) return props;

                var name = new string(s, keyStart, i - keyStart);
        ...
        private static readonly char[] KeywordSplitChars = new char[] { ' ', '\r', '\n', '\t', ';', ':', '.', ',' };
        ...
        public static bool KeywordSearch(this string value, string keywords)
        {
            ...
            var words = keywords.Split(KeywordSplitChars, StringSplitOptions.RemoveEmptyEntries);
            return words.Any(word => value.CaseInsensitiveContains(word));
        }
        ...
        public static bool Search(this string keywords, params string[] search)
        {
            ...
            var words = keywords.Split(KeywordSplitChars, StringSplitOptions.RemoveEmptyEntries);
            return words.Select(word => search.Where(find => !find.IsNullOrEmpty()).Any(find => find.CaseInsensitiveContains(word))).All(found => found);
            ...
            while (i < templateText.Length)
            {
                if (templateText[i] == '{')
                {
                    ...
                    if (templateText[i] == '}')
                    {
                        ...
                        var tokenName = templateText.Substring(start, i - start);

                        ++i;
                        var replText = getTokenReplacementText(tokenName);
        ...
        //    var formattedInput = string.Format("<root>{0}</root>", input);

        //    var inputBytes = Encoding.UTF8.GetBytes(formattedInput);
        ...
        {
            var sbResult = new StringBuilder(val.Length);
        ...
        {
            var length = cols.Length;
            var tabEncoded = new string[length];
            for (var i = 0; i < length; ++i)
            {
        ...
        public static string JoinTabDelimited(this object[] cols)
        {
            ...
            for (var i = 0; i < length; ++i)
            {
                ...
                {
                    var strValue = TypeDescriptor.GetConverter(col.GetType()).ConvertToInvariantString(col);
